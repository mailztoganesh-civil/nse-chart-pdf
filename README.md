...
- Both the NSE symbol list and Yahoo Finance price data are fetched client-side
  through a public CORS proxy (corsproxy.io, with allorigins.win as a fallback).
  These are free and unauthenticated, so they can be slow or rate-limited under
  heavy use — especially if you select a very large number of stocks at once.
